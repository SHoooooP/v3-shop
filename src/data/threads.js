...
function _addToThreadCreationQueue(fn) {
  threadCreationQueue = threadCreationQueue
    .then(fn)
    .catch(err => {
      console.error(`Erreur durant la création du ticket: ${err.message}`);
    });
...
async function createNewThreadForUser(user, opts = {}) {
  return _addToThreadCreationQueue(async () => {
    ...
    if (existingThread) {
      throw new Error("Echec de l'ouverture du ticket pour l'utilisateur car celui-ci a déjà un ticket d'ouvert !");
    }
    ...
    console.log(`[NOTE] Nouveau ticket créé ${channelName}`);
    ...
    try {
      ...
      console.error(`Erreur de création de ticket pour ${user.username}#${user.discriminator}!`);
      throw err;
    }
    ...
    infoHeaderItems.push(`AGE DU COMTPE **${accountAge}**`);
    ...
    if (userLogCount > 0) {
      infoHeader += `\n\nCet utilisateur a créer **${userLogCount}** précédent celui-ci. Utilisez \`${config.prefix}logs\` pour les voir.`;
    }
