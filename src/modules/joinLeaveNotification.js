...
module.exports = ({ bot }) => {
    ...
    if (config.notifyOnMainServerJoin) {
        bot.on("guildMemberAdd", async (guild, member) => {
            ...
            if (thread != null) {
                await thread.postSystemMessage(`***L'utilisateur a rejoint le discord : ${guild.name}.***`);
            }
        });
    }
    ...
    if (config.notifyOnMainServerLeave) {
        bot.on("guildMemberRemove", async (guild, member) => {
            ...
            const thread = await threads.findOpenThreadByUserId(member.id);
            if (thread != null) {
                await thread.postSystemMessage(`***L'utilisateur a quitté le discord : ${guild.name}.***`);
            }
        });
    }
};
