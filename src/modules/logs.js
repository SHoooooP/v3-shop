...
module.exports = ({ bot, knex, config, commands, hooks }) => {
  ...
  const logCmd = async (msg, args, _thread) => {
    ...
    const logUrl = await getLogUrl(thread);
    if (logUrl) {
      msg.channel.createMessage(`Ouvre le lien suivant pour avoir les logs du ticket #${thread.thread_number}:\n<${addOptQueryStringToUrl(logUrl, args)}>`);
      return;
    }

    const logFile = await getLogFile(thread);
    if (logFile) {
      msg.channel.createMessage(`Téléchage le fichier suivant pour accéder au log du ticket #${thread.thread_number}:`, logFile);
      return;
    }

    if (thread.status === THREAD_STATUS.OPEN) {
      msg.channel.createMessage(`La log n'est pas disponible, car le ticket est ouvert dans : <#${thread.channel_id}>`);
      return;
    }

    msg.channel.createMessage("La log de ce ticket n'est pas disponible");
  };
