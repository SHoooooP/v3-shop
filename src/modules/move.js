...
module.exports = ({ bot, knex, config, commands }) => {
  ...
  commands.addInboxThreadCommand("move", "<category:string$>", async (msg, args, thread) => {
    ...
    try {
      ...
      console.log(`['INFO']FAILED TO MOVE THREAD`);
      thread.postSystemMessage(`Erreur du move: ${e.message}`);
      return;
    }
    ...
    if (config.syncPermissionsOnMove) {
      ...
      try {
        ...
        thread.postSystemMessage(`Ticket move ${targetCategory.name.toUpperCase()}, mais impossible de synchroniser les paramètres: ${e.message}`);
        return;
      }
    }

    thread.postSystemMessage(`Ticket move ${targetCategory.name.toUpperCase()}`);
  });
};
