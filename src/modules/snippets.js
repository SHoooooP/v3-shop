...
module.exports = ({ bot, knex, config, commands }) => {
  ...
  commands.addInboxServerCommand("snippet", "<trigger> [text$]", async (msg, args, thread) => {
    ...
    if (snippet) {
      if (args.text) {
        ...
        utils.postSystemMessageWithFallback(msg.channel, thread, `Snippet "${args.trigger}" existe déjà. Vous pouvez le modifié grâce à : ${config.prefix}edit_snippet et ${config.prefix}delete_snippet respectivement.`);
      } else {
        // If the snippet exists and we're NOT trying to create a new one, show info about the existing snippet
        utils.postSystemMessageWithFallback(msg.channel, thread, `\`${config.snippetPrefix}${args.trigger}\` répondre avec: \`\`\`\n${utils.disableCodeBlocks(snippet.body)}\`\`\``);
      }
    } else {
      if (args.text) {
        ...
        utils.postSystemMessageWithFallback(msg.channel, thread, `Snippet "${args.trigger}" n'existe pas! Vous pouvez le créer avec : \`${config.prefix}snippet ${args.trigger} text\``);
      }
    }
  }, {
    aliases: ["s"]
  ...
  commands.addInboxServerCommand("delete_snippet", "<trigger>", async (msg, args, thread) => {
    const snippet = await snippets.get(args.trigger);
    if (! snippet) {
      utils.postSystemMessageWithFallback(msg.channel, thread, `Snippet "${args.trigger}" n'existe pas !`);
      return;
    }
  ...
  commands.addInboxServerCommand("snippets", [], async (msg, args, thread) => {
    ...
    utils.postSystemMessageWithFallback(msg.channel, thread, `Les snippets disponible sont :  (prefix ${config.snippetPrefix}):\n${triggers.join(", ")}`);
  }, {
    aliases: ["s"]
  });
};
